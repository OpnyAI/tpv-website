...
export function AnimatedBackground({
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "absolute inset-0 bg-[url('/images/backgrounds/tpv-background-lines.png')] bg-cover bg-center mix-blend-screen",
          variant === "video" ? "md:hidden" : "",
          intensityClasses[intensity],
        )}
      />
      <div className="absolute inset-0 bg-gradient-to-b from-tpv-deep/90 via-tpv-dark/86 to-tpv-deep" />
      <div className="absolute -left-24 top-20 h-72 w-72 rounded-full bg-tpv-magenta/20 blur-3xl" />
      <div className="absolute right-0 top-1/3 h-96 w-96 rounded-full bg-tpv-violet/18 blur-3xl" />
      <div className="absolute bottom-0 left-1/2 h-80 w-80 -translate-x-1/2 rounded-full bg-tpv-accent/12 blur-3xl" />
    </div>
  );
}
